...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QRadioButton, QGridLayout, QTextEdit
)
...
class DetectionApp(QWidget):
    ...
    def validate_roi(self, frame_width=1920, frame_height=1080):
        """
        ROI가 프레임 경계 내에 있는지 확인하고 경고창 표시

        Args:
            frame_width: 비디오 프레임의 너비
            frame_height: 비디오 프레임의 높이

        Returns:
            bool: ROI가 유효한 경우 True, 아닌 경우 False
        """
        # ROI 끝 좌표 계산
        ...
        if self.roi_x < 0 or self.roi_y < 0:
            error_message = "ROI 시작점이 프레임 경계를 벗어났습니다."
            is_valid = False
        elif roi_end_x > frame_width or roi_end_y > frame_height:
            error_message = f"ROI가 프레임 경계를 벗어났습니다.\n프레임 크기: {frame_width}x{frame_height}\nROI: (x={self.roi_x}, y={self.roi_y}, width={self.roi_width}, height={self.roi_height})"
            is_valid = False
        elif self.roi_width <= 10 or self.roi_height <= 10:
            error_message = "ROI 크기가 너무 작습니다. 최소 10x10 이상이어야 합니다."
            is_valid = False
